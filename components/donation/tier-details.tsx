...
export function TierDetails({ tier }: TierDetailsProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-[#1a1a4a]/80 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-[#FF0099]/20"
    >
      <div className="flex items-center mb-4">
        ...
        <div>
          ...
          <p className="text-gray-300 text-xs sm:text-sm">{tier.description}</p>
        </div>
      </div>

      <div className="mt-4">
        <h4 className="font-semibold text-white mb-3 text-sm sm:text-base">
          Pourquoi avons nous besoins de votre soutien ? :
        </h4>
        <ul className="space-y-2">
          {tier.benefits.map((benefit, index) => (
            <li key={index} className="flex items-start">
              <DynamicIcon
                name="check-circle"
                className="w-5 h-5 text-[#FF0099] mr-2 flex-shrink-0 mt-0.5"
              />
